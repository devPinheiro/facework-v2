...
const PostFeed = props => {
    ...
    const onSubmit = (data, { setSubmitting, setErrors, resetForm }) => {
        ...
        if (data.selectedVideo) {
            ...
            dispatch(createPostRequest(data)).then(response => {
                if (
                    response.payload.data.message ||
                    response.payload.data.error
                ) {
                ...
                <div className="w-full m-auto max-w-xl flex ">
                    <div className="w-full md:w-2/3 my-8 mx-4 ">
                        ...
                        {isLoading ? (
                            ...
                            feed.map((item, key) => (
                                <PostCard
                                    ...
                                    occupation={
                                        item.profile && item.profile.service
                                    }
                                    slug={item.id}
                        ...
                        {feedState.nextPage && (
                            <Button
                                type="button"
                                style="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-6 border border-gray-400 rounded shadow mr-4"
                                click={loadMorePosts}
                            >
                                Load More
                            </Button>
